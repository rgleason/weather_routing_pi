...
// Geometry port of catalogue v1.15.45.7,
// ed07e567c22e5c65d1711f7a4ded654101912043. Host/UI/weather dependencies
// removed; per-request ownership and bounded cancellation added. Contour
// intersection/merging algorithms retained.
#include "Contour.h"
...
IsoRoute::IsoRoute(SkipPosition* s, int dir)
    : arena(s->arena), skippoints(s), direction(dir), parent(nullptr) {
  arena.allocation();
  slot = arena.routes.add(this);
...
IsoRoute::~IsoRoute() {
  arena.routes.remove(slot);
  if (arena.cleaning) return;
  for (IsoRouteList::iterator it = children.begin(); it != children.end(); ++it)
    delete *it;
...
void IsoRoute::FindIsoRouteBounds(double bounds[4]) {
  ...
  while (s != skippoints) {
    p = s->point;
    bounds[MINLAT] = std::min(p->lat, bounds[MINLAT]);
    bounds[MAXLAT] = std::max(p->lat, bounds[MAXLAT]);
    bounds[MINLON] = std::min(p->lon, bounds[MINLON]);
    bounds[MAXLON] = std::max(p->lon, bounds[MAXLON]);
...
bool Normalize(IsoRouteList& rl, IsoRoute* route1, IsoRoute* route2, int level,
  ...
  bool normalizing;
  Arena& operationArena = route1->arena;
  if (level > 64)
    throw Stop{wr::RoutingStatus::ResourceLimitReached,
               "contour nesting limit"};

reset:
  operationArena.checkpoint();
...
bool Merge(IsoRouteList& rl, IsoRoute* route1, IsoRoute* route2, int level,
  ...
  if (route1->direction == -1 && route2->direction == -1) {
    ...
    throw Stop{wr::RoutingStatus::InternalError,
               "invalid inverted contour merge"};
  }
...
SkipPosition::SkipPosition(Position* p, int q)
    : arena(p->arena), point(p), quadrant(q) {
  arena.allocation();
  slot = arena.skips.add(this);
}
SkipPosition::~SkipPosition() { arena.skips.remove(slot); }
