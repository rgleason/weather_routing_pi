...
double rad2posdeg(double radians) { return positive_degrees(rad2deg(radians)); }
...
wxString calculateTimeDelta(wxDateTime startTime, wxDateTime endTime) {
  ...
  if (startTime.IsValid() && endTime.IsValid()) {
    ...
    timeStr =
        (days ? wxString::Format(_T("%dd "), days) : wxString()) +
        (hours || days
...
char* strtok_r(char* str, const char* delim, char** save) {
  ...
  if (!save) return strtok(str, delim);
  if (!str && !(str = *save)) return NULL;
  last = str + strlen(str);
