...
void LineBuffer::pushTransformedBuffer(LineBuffer& buffer, int x, int y,
                                       double ang, bool south, bool head,
                                       int lineWidth) {
  // transform vertexes by angle
  float sa = sinf(ang), ca = cosf(ang);

  float m[2][2] = {{ca, -sa}, {sa, ca}};
  ...
  for (int i = 0; i < 2 * buffer.count; i += 2) {
    ...
    pushLine(k[0] * m[0][0] + k[1] * m[0][1] + x,
             k[0] * m[1][0] + k[1] * m[1][1] + y,
             k[2] * m[0][0] + k[3] * m[0][1] + x,
             k[2] * m[1][0] + k[3] * m[1][1] + y);
  }
}
...
void WindBarbLineBuffer::pushTriangle(int b, int lineWidth) {
  pushLine(0, b, -(10 * (lineWidth / DEFAULT_WIND_ARROW_SIZE_FACTOR)), b - 4);
  pushLine(0, b - 8, -(10 * (lineWidth / DEFAULT_WIND_ARROW_SIZE_FACTOR)),
           b - 4);
}
...
void LineBufferOverlay::setLineBuffer() {
  ...
  const int windArrowSize =
      (int)(DEFAULT_WIND_ARROW_SIZE *
            (m_lineWidth / DEFAULT_WIND_ARROW_SIZE_FACTOR));
  ...
  for (i = 0; i < 14; i++) {
    ...
    if (i >= 10) {
      ...
      m_SingleArrow[i].pushLine(0, dec, /* */ -dec, dec - arrowSize);
      m_SingleArrow[i].pushLine(dec, dec - arrowSize, /* */ 0,
                                dec - arrowSize + h);
      m_SingleArrow[i].pushLine(-dec, dec - arrowSize, /* */ 0,
                                dec - arrowSize + h);
    } else {
  ...
  for (i = 0; i < 14; i++) m_SingleArrow[i].Finalize();
}

void LineBufferOverlay::pushWindArrowWithBarbs(LineBuffer& buffer, int x, int y,
                                               double vkn, double ang,
                                               bool south, bool head) {
  int cacheidx;
...
void LineBufferOverlay::pushSingleArrow(LineBuffer& buffer, int x, int y,
                                        double vkn, double ang, bool south) {
  int cacheidx = 13;
  int v = (int)(vkn * 2. + 0.5);
