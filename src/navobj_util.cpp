      ...
      if ((flags & OUT_GUID) || (flags & OUT_VIZ) || (flags & OUT_VIZ_NAME) ||
          ...
          if ((flags & OUT_RTE_PROPERTIES) &&
              ...
              if (pr->GetPlannedSpeed() > 0.0001) {
                  pugi::xml_attribute use =
     child.append_attribute("planned_speed"); use.set_value(
                          wxString::Format(_T("%.1lf"),
     pr->GetPlannedSpeed()).mb_str());
              }
              if (pr->m_manual_etd) {
                  pugi::xml_attribute use = child.append_attribute("etd");
...
static bool GPXCreateRoute(pugi::xml_node node, const SimpleRoute& Route) {
  ...
  */
  for (size_t i = 0; i < Route.m_vec_points.size(); i++) {
    GPXCreateWpt(node.append_child("rtept"), Route.m_vec_points[i]);
  }
  /*
...
SimpleRoutePoint::SimpleRoutePoint(double lat, double lon,
...
SimpleNavObjectXML::SimpleNavObjectXML() : pugi::xml_document() {
