...
void* RouteMapOverlayThread::Entry() {
  ...
  wxLogMessage("DEBUG: Entry() initial Finished() = %d", m_RouteMapOverlay.Finished());

  wxLogMessage(
      "OverlayState: overlay=%p Stopped=%d ThreadAlive=%d ThreadExited=%d "
      "Finished=%d ReachedDest=%d",
      &m_RouteMapOverlay, m_RouteMapOverlay.m_Stopped,
      m_RouteMapOverlay.m_bThreadAlive.load(),
      m_RouteMapOverlay.m_bThreadExited.load(), m_RouteMapOverlay.Finished(),
      m_RouteMapOverlay.ReachedDestination());
  ...
  if (!cf.RouteGUID.IsEmpty()) {
    ...
    while (!TestDestroy() && !m_RouteMapOverlay.m_Stopped) {
  
       wxLogMessage(
          "WorkerLoop: LOOP OverlayState: overlay=%p Stopped=%d ThreadAlive=%d "
          "ThreadExited=%d Finished=%d ReachedDest=%d",
          &m_RouteMapOverlay, m_RouteMapOverlay.m_Stopped,
          m_RouteMapOverlay.m_bThreadAlive.load(),
          m_RouteMapOverlay.m_bThreadExited.load(),
          m_RouteMapOverlay.Finished(), m_RouteMapOverlay.ReachedDestination());

      if (!m_RouteMapOverlay.Propagate())
         // No more work possible: exit workerloop
         //  wxThread::Sleep(50);
         break;
      else {
  ...
  wxLogMessage(
         "WorkerLoop NORMAL EXIT: OverlayState: overlay=%p Stopped=%d ThreadAlive=%d "
         "ThreadExited=%d Finished=%d ReachedDest=%d",
          &m_RouteMapOverlay, m_RouteMapOverlay.m_Stopped,
          m_RouteMapOverlay.m_bThreadAlive.load(),
          m_RouteMapOverlay.m_bThreadExited.load(), m_RouteMapOverlay.Finished(),
...
/* Monitoring and logging of thread exit state only.
 * No logic changes here. Not where you determine status.
 * wxWidgets requires thread object is destroyed by the parent, not the thread itself
 * Thread signals its done, parent waits for signal, 
 * Then parent deletes thread object.
...
void RouteMapOverlayThread::OnExit() {
  {
   ...
   m_RouteMapOverlay.m_bThreadAlive.store(false, std::memory_order_release);
   m_RouteMapOverlay.m_bThreadExited.store(true, std::memory_order_release);
 
   wxLogMessage(
     "EngineState:OnExit(): FINAL STATE Finished=%d "
     "ReachedDest=%d Stopped=%d ThreadAlive=%d ThreadExited=%d",
     m_RouteMapOverlay.Finished(),
     m_RouteMapOverlay.ReachedDestination(),
     m_RouteMapOverlay.m_Stopped,
     m_RouteMapOverlay.m_bThreadAlive.load(),
     m_RouteMapOverlay.m_bThreadExited.load());
  }
...
void RouteMapOverlay::SetConfiguration(const RouteMapConfiguration& cfg) {
  ...
  RouteMap::SetFinished(false);

  // If RouteMapOverlay has its own copy, assign it here:
  // Check your header: if you see "RouteMapConfiguration m_Configuration;"
  // inside RouteMapOverlay, keep this line. If not, delete it.
...
bool RouteMapOverlay::Start(wxString& error) {
  {
    ...
    wxLogMessage(
        "OverlayState: overlay=%p Stopped=%d ThreadAlive=%d ThreadExited=%d "
        "Finished=%d ReachedDest=%d",
        this, m_Stopped, m_bThreadAlive.load(), m_bThreadExited.load(),
        Finished(), ReachedDestination());



    // ---- FULL RESET OF ROUTEMAP ENGINE STATE ----
    Lock();  // RouteMap internal mutex

    RouteMap::Reset();  // Clear existing route data

  //  Let RoueMap handle these settings internally.
  //  SetFinished(false);   // m_bFinished = false;
  //  SetValid(true);    //  m_bValid = true;
  //  SetWeatherForecastStatus(WEATHER_FORECAST_OK);  //m_bWeatherForecastStatus
  //  SetReachedDestination(false);  // m_bReachedDestination = false;
  //  SetNewTime(wxDateTime());    // m_NewTime = wxDateTime(); CRITICAL: reset GRIB time cursor
  //  SetNeedsGrib(false);    // m_bNeedsGrib = false;  Clearing Grib State
  //  origin.clear();
  //  m_NewGrib = nullptr;
  //  m_SharedNewGrib.SetGribRecordSet(nullptr);
    ...
    // (your validation checks remain unchanged)
  ...
  while (!m_bThreadAlive.load(std::memory_order_acquire)) {
    if (std::chrono::steady_clock::now() - start > std::chrono::seconds(2)) {
      wxLogMessage(
          "RouteMapOverlay::Start: thread failed to signal alive within timeout");
  ...
  wxLogMessage(
      "OverlayState: Start() COMPLETE  overlay=%p Stopped=%d ThreadAlive=%d ThreadExited=%d "
      "Finished=%d ReachedDest=%d",
      this, m_Stopped, m_bThreadAlive.load(), m_bThreadExited.load(),
      Finished(), ReachedDestination());
...
void RouteMapOverlay::DeleteThread() {
  // Legacy API: Stop() is now the canonical shutdown.
  // Keep this as a no-op or a thin wrapper that does NOT touch the thread.
  wxLogMessage(
      "RouteMapOverlay::DeleteThread() called; Stop() is canonical, no action "
      "taken.");
}
...
/* Stop is the single place that joins and deletes the thread. Still wait.
 * Single owner of Delete(), Wait(), and delete.
 * never reset m_bThreadExited in Stop(). That flag is owned by the worker thread.
 */
void RouteMapOverlay::Stop() {
  wxLogMessage(
      "OverlayState: Stop()  overlay=%p Stopped=%d ThreadAlive=%d "
      "ThreadExited=%d "
      "Finished=%d ReachedDest=%d",
      this, m_Stopped, m_bThreadAlive.load(), m_bThreadExited.load(),
      Finished(), ReachedDestination());

  if (m_bThreadExited.load(std::memory_order_acquire) && !m_Thread) return;
...

  {
    ...
    // If the worker thread has already exited, we just need to join & delete.
    already_exited = m_bThreadExited.load(std::memory_order_acquire);


    // Signal worker thread to exit if still running
    if (!already_exited)
    ...
    thread = m_Thread;
    m_Thread = nullptr;  //Stop() is now the sole owner
  }

 // If not already exited, request termination
  if (!already_exited) {
    thread->Delete();
  ...
  // At this point the thread should be finished; Wait() once, then delete.
  thread->Wait();
  delete thread;

  wxLogMessage(
      "OverlayState: Stop() COMPLETE  overlay=%p Stopped=%d ThreadAlive=%d "
      "ThreadExited=%d Finished=%d ReachedDest=%d",
      this, m_Stopped, m_bThreadAlive.load(), m_bThreadExited.load(),
      Finished(), ReachedDestination());
...
/* RouteMapOverlay::SetFinished is a thin wrapper around
the engine?level RouteMap::SetFinished(f).
Meaning:
The routing engine sets its internal m_bFinished flag
The overlay does not maintain its own finished flag here
The UI reads the overlay?s Finished() which forwards to the engine

So this wrapper is the perfect place to log:
When the overlay requests the engine to mark finished
