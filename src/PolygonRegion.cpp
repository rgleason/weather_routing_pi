...
PolygonRegion::PolygonRegion(int n, float* points) {
  ...
  if (!contour.CCW()) contour.Reverse();
...
TESStesselator* PolygonRegion::Tesselate(bool triangles) {
  ...
  if (triangles)
    ...
    success = tessTesselate(tess, TESS_WINDING_POSITIVE, TESS_BOUNDARY_CONTOURS,
                            0, 0, 0);
...
void PolygonRegion::Put(const PolygonRegion& region, int winding_rule,
                        bool reverse) {
  TESSalloc ma;
  //    ma.memalloc = poolAlloc;
  //    ma.memfree = poolFree;
