...
if (NOT QT_ANDROID)
  ...
  # ============================================================
  # wxWidgets detection (CONFIG preferred → vcpkg → legacy as fallback)
  # ============================================================
  set(CMLOC "[PluginConfigure]")
  set(USE_WX_CONFIG_MODE OFF)
    # Try CONFIG-mode...
    # If found → set(USE_WX_CONFIG_MODE TRUE)
    # If still FALSE → run legacy FindwxWidgets
	
	# On MSVC, skip CONFIG-mode entirely and force legacy
	# Begin MSVC guard around CONFIG-mode
  if (MSVC)  
    message(STATUS "${CMLOC}MSVC: forcing legacy FindwxWidgets (no CONFIG-mode)")
  else()
  
    # 1) Try CMake CONFIG-mode ( Uses wxWidgetsConfig.cmake file)
    ...
    # 2)  Optional: vcpkg / system CONFIG-mode fallback
  ...
  endif()  # end MSVC guard around CONFIG-mode
  # ------------------------------------------------------------
  # 3) Legacy FindwxWidgets (Linux, MinGW, macOS, MSVC fallback)
  # ------------------------------------------------------------
  if (NOT USE_WX_CONFIG_MODE)
    message(STATUS "${CMLOC}Using legacy FindwxWidgets")

    # MSVC FIX: force correct include + lib directories
    if (MSVC)
        set(wxWidgets_LIB_DIR "${wxWidgets_ROOT_DIR}/lib/vc_dll")
        set(wxWidgets_INCLUDE_DIRS "${wxWidgets_ROOT_DIR}/lib/vc_dll/mswud")

        # Exclude wxWidgets' bundled expat/jpeg/png/tiff/regex libs
        # These conflict with OpenCPN’s own libraries
	...
	# Link wxWidgets libraries into the plugin target
    target_link_libraries(${PACKAGE_NAME} PRIVATE ${wxWidgets_LIBRARIES})

	
	# wxWidgets internal zlib (needed for zuFile.cpp)
    if (WIN32 AND MSVC)
      list(APPEND wxWidgets_LIBRARIES
         debug     "${wxWidgets_ROOT_DIR}/lib/vc_dll/wxzlibd.lib"
         optimized "${wxWidgets_ROOT_DIR}/lib/vc_dll/wxzlib.lib"
      )
    endif()
    ...
    # ------------------------------------------------------------
    # wxWidgets dual‑mode compatibility layer:
    # synthesize wx:: targets for legacy builds
    # ------------------------------------------------------------
    set(_wx_components
      base core net xml html adv aui gl
    )

    foreach(_comp IN LISTS _wx_components)
      if (NOT TARGET wx::${_comp})
        add_library(wx::${_comp} INTERFACE IMPORTED)
        # In legacy mode we don't have per‑component libs, so just
        # link the full wxWidgets_LIBRARIES set. Slightly over‑broad,
        # but safe and simple.
    ...
    # ------------------------------------------------------------
    # Link wxWidgets' internal zlib (MSVC only) using find_library
    # ------------------------------------------------------------
    if (MSVC)

        # === DIAGNOSTIC: SHOW EXACT PATH AND FILES CMAKE SEES ===
        message(STATUS "wxWidgets_ROOT_DIR = ${wxWidgets_ROOT_DIR}")
        message(STATUS "Checking for zlib in: ${wxWidgets_ROOT_DIR}/lib/vc_dll")

        file(GLOB ZLIB_CANDIDATES
            "${wxWidgets_ROOT_DIR}/lib/vc_dll/*zlib*.lib"
        )

        message(STATUS "Zlib candidates found: ${ZLIB_CANDIDATES}")
        # === END DIAGNOSTIC ===

        find_library(WX_ZLIB_LIB
            NAMES wxzlibd wxzlib
            PATHS "${wxWidgets_ROOT_DIR}/lib/vc_dll"
        )
        if (WX_ZLIB_LIB)
            target_link_libraries(${PACKAGE_NAME} PRIVATE ${WX_ZLIB_LIB})
        endif()
    endif()
  ...
  endif()  #  end legacy block  NOT USE_WX_CONFIG_MODE

    
  # ------------------------------------------------------------
  # 4) Link wxWidgets via wx:: targets (CONFIG or legacy)
  # ------------------------------------------------------------
    ...
    message(STATUS "${CMLOC}Final wxWidgets Libraries: ${wxWidgets_LIBRARIES}")
  
   
# ------------------------------------------------------
#[[ Added new wxwidgets section down to here ]]
# ------------------------------------------------------
...
endif () # END wxWidgets + Android block
